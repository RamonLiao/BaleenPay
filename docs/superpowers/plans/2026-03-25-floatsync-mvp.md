# FloatSync MVP Implementation Plan
...
**Architecture:** Single Move package (`floatsync`) with 5 modules (brand_usd, merchant, payment, router, events). Next.js frontend with Checkout Widget + Merchant Dashboard. PTB composition pattern decouples payment recording from fund routing.

**Tech Stack:** SUI Move (2024 Edition), Next.js 15, shadcn/ui, @mysten/dapp-kit, @mysten/sui, TypeScript 5, Vitest, Playwright

**Spec:** `docs/superpowers/specs/2026-03-25-floatsync-mvp-design.md` (v1.2)

---

## File Structure

### Move Contract (`move/floatsync/`)
...
**Files:**
- Create: `move/floatsync/Move.toml`
- Create: `move/floatsync/sources/` (empty dir)
- Create: `move/floatsync/tests/` (empty dir)

- [ ] **Step 1: Create Move project structure**

```bash
mkdir -p move/floatsync/sources move/floatsync/tests
```
...
move/floatsync/build/
```

- [ ] **Step 5: Commit**

```bash
git add move/floatsync/ .gitignore
git commit -m "chore: init Move project structure for floatsync"
```
...
**Files:**
- Create: `move/floatsync/sources/events.move`
- Create: `move/floatsync/sources/brand_usd.move`
...
```bash
git add move/floatsync/sources/events.move move/floatsync/sources/brand_usd.move move/floatsync/tests/test_usdc.move
...
**Files:**
- Create: `move/floatsync/sources/merchant.move`
- Create: `move/floatsync/tests/merchant_tests.move`

- [ ] **Step 1: Write merchant_tests.move — test register_merchant**

```move
#[test_only]
module floatsync::merchant_tests {
...
Run: `cd move/floatsync && sui move test --filter merchant_tests`
...
```bash
git add move/floatsync/sources/merchant.move move/floatsync/tests/merchant_tests.move
...
**Files:**
- Create: `move/floatsync/sources/payment.move`
- Create: `move/floatsync/tests/payment_tests.move`

- [ ] **Step 1: Write payment_tests.move — test pay_once**

```move
#[test_only]
module floatsync::payment_tests {
    use floatsync::merchant;
...
Run: `cd move/floatsync && sui move test --filter payment_tests`
...
```bash
git add move/floatsync/sources/payment.move move/floatsync/tests/payment_tests.move
...
- Create: `move/floatsync/tests/subscription_tests.move`
...
Run: `cd move/floatsync && sui move test --filter subscription_tests`
Expected: FAIL
...
```bash
git add move/floatsync/sources/payment.move move/floatsync/tests/subscription_tests.move
...
**Files:**
- Create: `move/floatsync/sources/router.move`
- Create: `move/floatsync/tests/router_fallback_tests.move`
...
Run: `cd move/floatsync && sui move test --filter router_fallback`
Expected: FAIL
...
```bash
git add move/floatsync/sources/router.move move/floatsync/sources/merchant.move move/floatsync/tests/router_fallback_tests.move move/floatsync/tests/merchant_tests.move
...
**Files:**
- Create: `move/floatsync/tests/integration_tests.move`
...
```bash
git add move/floatsync/tests/integration_tests.move
...
echo "Building..."
cd move/floatsync && sui move build
...
```bash
# Move
cd move/floatsync && sui move build && sui move test
