# FloatSync Demo App — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a 5-page Next.js demo app (Landing, Checkout, Subscribe, Dashboard, Developers) that showcases FloatSync payment infrastructure on SUI testnet with real wallet transactions.

**Architecture:** Next.js 15 App Router with Tailwind 4, consuming `@floatsync/sdk` and `@floatsync/react` workspace packages. Provider stack: WalletProvider → QueryClientProvider → FloatSyncProvider. All pages are client-rendered (wallet interactions require browser). Ocean-themed visual design (Stripe layout × SUI palette).

**Tech Stack:** Next.js 15, React 19, Tailwind CSS 4, @floatsync/sdk (workspace), @floatsync/react (workspace), @mysten/dapp-kit-react ^2.0.0, @mysten/sui ^2.8.0, @tanstack/react-query ^5

**Spec:** `docs/superpowers/specs/2026-03-26-floatsync-demo-app-design.md`

**Testnet Config (from `move/floatsync/deployed.testnet.json`):**
...
**Note:** `@mysten/dapp-kit-react` v2 uses `createNetworkConfig` + `SuiClientProvider` + `WalletProvider` as its provider stack. The `FloatSyncProvider` wraps inside so it can access the SUI client context.
...
export default function LandingPage() {
  return (
    <main className="flex min-h-screen items-center justify-center">
      <h1 className="text-4xl font-bold text-ocean-water">FloatSync Demo</h1>
    </main>
  )
}
```
...
```bash
cd /Users/ramonliao/Documents/Code/Project/Web3/BlockchainDev/SUI/Projects/FloatSync
pnpm install
pnpm --filter @floatsync/sdk build
pnpm --filter @floatsync/react build
pnpm --filter @floatsync/demo typecheck
```
...
```bash
pnpm --filter @floatsync/demo dev
```

Expected: Next.js dev server on http://localhost:3100, renders "FloatSync Demo" text.
  ...
  return (
    <footer className="bg-ocean-deep text-ocean-foam/60 py-12">
      <div className="mx-auto max-w-6xl px-6 flex items-center justify-between">
        <p className="text-sm">FloatSync — Payments infrastructure for the onchain economy</p>
...
```ts
export const HERO_CODE_SNIPPET = `import { FloatSync } from '@floatsync/sdk'
...
pnpm --filter @floatsync/sdk build
pnpm --filter @floatsync/react build
pnpm --filter @floatsync/demo build
```
...
```bash
pnpm --filter @floatsync/sdk test
pnpm --filter @floatsync/react test
```
