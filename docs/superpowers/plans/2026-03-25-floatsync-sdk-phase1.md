# FloatSync SDK Phase 1 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Deliver `@floatsync/sdk` + `@floatsync/react` + contract Order ID upgrade as a production-ready monorepo, incorporating all review findings.
...
| JSON-RPC deprecation — need rpcUrl config | WARNING | Task 4 (FloatSyncConfig) |
...
├── sdk/                          # @floatsync/sdk
│   ├── src/
│   │   ├── client.ts             # FloatSync main class
...
│   ├── package.json
│   └── tsconfig.json
├── react/                        # @floatsync/react
│   ├── src/
│   │   ├── provider.tsx          # <FloatSyncProvider>
│   │   ├── components/
│   │   │   ├── CheckoutButton.tsx
│   │   │   ├── PaymentForm.tsx
│   │   │   ├── SubscribeButton.tsx
...
**Files:**
- Modify: `move/floatsync/sources/events.move`
- Modify: `move/floatsync/sources/merchant.move`
...
Run: `cd move/floatsync && sui move build`
Expected: SUCCESS (no errors)

- [ ] **Step 7: Commit**

```bash
git add move/floatsync/sources/events.move move/floatsync/sources/merchant.move
...
**Files:**
- Modify: `move/floatsync/sources/payment.move`
...
```bash
git add move/floatsync/sources/payment.move
...
**Files:**
- Create: `move/floatsync/tests/payment_v2_tests.move`
- Create: `move/floatsync/tests/monkey_v2_tests.move`
...
```move
#[test_only]
module floatsync::payment_v2_tests {
...
Run: `cd move/floatsync && sui move test --filter payment_v2`
Expected: PASS
...
Run: `cd move/floatsync && sui move test`
...
```move
#[test_only]
module floatsync::monkey_v2_tests {
    use sui::test_scenario;
    use sui::coin;
    use sui::clock;
    use std::string;
...
```bash
git add move/floatsync/tests/payment_v2_tests.move move/floatsync/tests/monkey_v2_tests.move
...
Run: `cd /path/to/floatsync && pnpm install && pnpm --filter @floatsync/sdk typecheck`
Expected: SUCCESS
...
Run: `pnpm --filter @floatsync/sdk test`
Expected: ALL PASS
...
```typescript
export class FloatSyncError extends Error { code: string; ... }
...
// parseAbortCode(status) → FloatSyncError with human message from ABORT_CODE_MAP
```
...
- [ ] **Step 2: Write FloatSync client class**

`packages/sdk/src/client.ts`:
- Constructor takes `FloatSyncConfig`, creates `SuiClient`
...
- Extends or wraps FloatSync with admin-only ops: `pause()`, `unpause()`, `setRouterMode()`
...
Run: `pnpm --filter @floatsync/sdk test && pnpm --filter @floatsync/sdk typecheck`
Expected: ALL PASS

- [ ] **Step 8: Build SDK**

Run: `pnpm --filter @floatsync/sdk build`
...
git commit -m "feat(sdk): FloatSync client + AdminClient + idempotency guard"
```
...
## Task 9: React — Package Setup + Provider + useFloatSync
...
- Create: `packages/react/src/hooks/useFloatSync.ts`
...
- [ ] **Step 2: Write FloatSyncProvider + useFloatSync hook**

Provider creates SDK client from config, stores in React context. `useFloatSync()` returns client instance.

- [ ] **Step 3: Write tests**

Test provider renders, useFloatSync returns client, missing provider throws.

- [ ] **Step 4: Run tests**

Run: `pnpm --filter @floatsync/react test`
Expected: ALL PASS
...
git commit -m "feat(react): FloatSyncProvider + useFloatSync hook"
```
...
Run: `pnpm --filter @floatsync/react test && pnpm --filter @floatsync/react build`
...
**Files:**
- Modify: `move/floatsync/deployed.testnet.json` (update with new digest)
...
```bash
git add move/floatsync/deployed.testnet.json
...
| 9 | React | 5 | Provider + useFloatSync |
