# BaleenPay MVP Design Specification
...
BaleenPay is a white-label stablecoin payment widget for SaaS platforms on the SUI blockchain. Users pay USDC, which is routed through StableLayer to mint BrandUSD and auto-deposit into yield pools. Merchants earn yield on idle payment float and can claim it anytime.
...
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │              baleenpay package (single package)          │    │
│  │                                                         │    │
...
module baleenpay::brand_usd {
    ...
    fun init(witness: BRAND_USD, ctx: &mut TxContext) {
        let (treasury_cap, metadata) = coin::create_currency(
            ...
            b"BaleenPay branded stablecoin", option::none(), ctx
        );
...
<iframe
  src="https://baleenpay.app/checkout/0xMERCHANT_ACCOUNT_ID?plan=subscription&amount=10&period=month"
  width="400"
  height="600"
/>

<!-- Future: npm package -->
<!-- <BaleenPayCheckout merchantId="0x..." plan="subscription" amount={10} period="month" /> -->
```
...
- `baleenpay_core` (merchant + payment) — low change frequency
- `baleenpay_router` (router + treasury) — high change frequency
...
```bash
npm install @baleenpay/checkout
```

```tsx
import { BaleenPayCheckout } from '@baleenpay/checkout';
<BaleenPayCheckout merchantId="0x..." network="mainnet" />
```
...
| 4 | Domain registration (baleenpay.app) and GitHub org | TODO: manual |
