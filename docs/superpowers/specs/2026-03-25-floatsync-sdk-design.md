# FloatSync SDK Design Specification

> **Version:** 1.0
> **Date:** 2026-03-25
> **Status:** Draft
> **Scope:** Phase 1 — `@floatsync/sdk` + `@floatsync/react` + Contract Order ID Upgrade

---

## 1. Executive Summary

FloatSync SDK is a Stripe-inspired developer toolkit for SUI blockchain payments. It provides a progressive integration model targeting both Web3 native and Web2 developers, enabling SaaS platforms to embed stablecoin payments with minimal effort.

**Phase 1 delivers:**
- `@floatsync/sdk` — TypeScript SDK with PTB builders, event listeners, coin registry, idempotency
- `@floatsync/react` — React components (CheckoutButton, PaymentForm, SubscribeButton) + hooks
...
- Phase 2: `@floatsync/server` (webhook relay, server-side idempotency)
- Phase 3: FloatSync Cloud (REST API, hosted checkout, API key management, usage metering)
...
```
              ...
              ┌──────┘                     └──────┐
              ▼                                   ▼
    @floatsync/sdk (core)              @floatsync/react
    ┌─────────────────────┐           ┌──────────────────┐
    │ FloatSync client    │◄──────────│ <FloatSyncProvider>│
    │   .pay()            │           │ <CheckoutButton>  │
...
@floatsync/sdk/
├── src/
│   ├── client.ts          # FloatSync main class — single entry point
...
class FloatSync {
  ...
  // ── Events ──
  on(event: FloatSyncEvent, callback: EventCallback): Unsubscribe
}
```
...
const fs = new FloatSync({
  ...
  packageId: '0x7d12...5097',   // floatsync package
...
Returning `Transaction` (not executing) allows Web3 users to compose FloatSync PTBs with other operations in a single atomic transaction.
...
- **Scope:** in-memory, per `FloatSync` instance (resets on page refresh)
...
## 5. React Components (`@floatsync/react`)
...
│   │   └── MerchantBadge.tsx
│   ├── hooks/
│   │   ├── useFloatSync.ts     # Get SDK client instance
...
```tsx
import { FloatSyncProvider, CheckoutButton } from '@floatsync/react'
...
- **Optional: `theme="default"`** — FloatSync branded styles
...
```
floatsync/
├── move/floatsync/          # Move contracts (existing + order_id upgrade)
├── packages/
│   ├── sdk/                 # @floatsync/sdk
...
|---------|--------|--------|
| `@floatsync/sdk` | ESM + CJS + types (tsup) | Browser + Node.js |
| `@floatsync/react` | ESM + types (tsup) | Browser only |
...
|---------------|-------------|
| Checkout widget | `@floatsync/react` CheckoutButton + PaymentForm |
| Merchant dashboard | Demo app using SDK merchant hooks |
| lib/sui/ abstraction | `@floatsync/sdk` (more general, reusable) |
