# BaleenPay Demo App — Design Spec

## 1. Purpose & Audience

**Primary audience:** Investors and potential partners evaluating BaleenPay as a Web3 payment infrastructure play. They need to see a polished, working product — not a prototype.

**Secondary audience:** Developers already using Stripe who are curious about Web3 payments. They need to see that integrating BaleenPay is as simple as Stripe.
...
**Purpose:** Show developers how easy BaleenPay is to integrate. Code-as-marketing.
...
apps/demo/
├── app/
│   ├── layout.tsx            # Providers: Wallet + QueryClient + BaleenPay
│   ├── page.tsx              # Landing
│   ├── checkout/page.tsx
│   ├── subscribe/page.tsx
│   ├── dashboard/page.tsx
...
<WalletProvider>              # @mysten/dapp-kit-react
  <QueryClientProvider>       # @tanstack/react-query
    <BaleenPayProvider>       # @baleenpay/react
      {children}
    </BaleenPayProvider>
  </QueryClientProvider>
</WalletProvider>
...
- Depends on `@baleenpay/sdk` and `@baleenpay/react` via workspace protocol
...
The demo uses the already-deployed v2 contract on SUI testnet. All object IDs from `move/baleenpay/deployed.testnet.json`. No additional deployment needed.
...
- Domain: TBD (e.g., `demo.baleenpay.io` or Vercel subdomain)
